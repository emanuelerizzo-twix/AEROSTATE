...
class Project:
    ...
    # Connections are defined on the flattened bay order (wing order, then bay order)
...
# -----------------------------
# Derived sections from bay geometry
# -----------------------------
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.props_dock.resize(360, 600)
    ...
    def _apply_constraints_to_models(self):
        ...
        # Map flat index -> BayModel
        bay_by_flat = {k: flats[k][2] for k in range(len(flats))}

        # For determinism: sort constraints by slave then master then ctype
        cons = sorted(self.project.connections, key=lambda c: (c.slave_index, c.master_index, c.ctype))

        # Airfoil rule: if any connection exists for a slave, then airfoil root(slave)=airfoil tip(master)
        # We'll apply after each constraint set, using the first master found for that slave.
        ...
        for c in cons:
            ...
            # Minimal attach affects only position Y,Z (not X)
        ...
        for slave_idx, master_idx in first_master_for_slave.items():
            if master_idx in bay_by_flat and slave_idx in bay_by_flat:
                ...
                # root of slave equals tip of master
                s.sections[0].airfoil = m.sections[-1].airfoil

        # Update derived sections for all bays
        for _, _, b in flats:
            update_default_sections_from_bay(b)
